...
//------------------------------------------------------------------------------
export const getWindDataFromPixel = function(pixel) {
  const [r, g, b] = pixel;
...
L.WindyLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _initWindy: function _initWindy(self) {
    ...
    self._windy = new Windy(options); // prepare context global var, start drawing
...
const Windy = function Windy(params) {
  ...
  const getWindDataFromCanvas = function (canvas) {
    const ctx = canvas.getContext('2d');
    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height)
    return buildGridParticles(imageData)
  }
  const buildGridParticles = function(imgData) {
    const particles = []
    const data = imgData.data;
    const width = imgData.width;
    const zoom = params.map.getZoom()
    let diapason = {a: 25, b: 25}
    if (zoom < 6) {
      diapason.a = 11
      diapason.b = 15
    }
  ...
  function interpolateGrid(grid) {
    ...
    for (let y = 0; y < grid.length - 8; y++) {
  
      for (let x = 0; x < grid[y].length - 8; x++) {
        const q00 = grid[y][x];
        const q01 = grid[y][x + 8];
        const q10 = grid[y + 8][x];
        const q11 = grid[y + 8][x + 8];
  ...
  function bilinearInterpolation(x, y, x0, y0, x1, y1, q00, q01, q10, q11) {
    ...
    const q0 = (fx0 * q00[0] + fx1 * q01[0]) / (x1 - x0);
    const q1 = (fx0 * q10[0] + fx1 * q11[0]) / (x1 - x0);
  
    return [(fy0 * q0 + fy1 * q1) / (y1 - y0), q00[1]];
  }
  const drawCanvasTiles = function() {

    const bounds = params.map.getBounds();

    const zoom = params.map.getZoom();

    const topLeft = params.map.project(bounds.getNorthWest(), zoom);
    const bottomRight = params.map.project(bounds.getSouthEast(), zoom);
  ...
  const animate = function animate(grid, particles) {
